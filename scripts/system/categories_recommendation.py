...
class CategoryRecommendationModule():
    ...
    def recommend_categories(self, category, length, k):
        ...
        # Ensure there are at least k items to sample
        if len(recommended_categories) < k:
            k = len(recommended_categories)
        # Sample k values in recommended_categories list
        recommended_categories = random.sample(recommended_categories, k)
